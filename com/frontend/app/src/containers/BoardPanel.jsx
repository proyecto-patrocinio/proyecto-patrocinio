...
/**
 * BoardPanel component represents a panel within a board, displaying a title and a list of cards.
 *
 ...
 * @returns {JSX.Element} - A React element representing the BoardPanel.
 */
const BoardPanel = ({ panel, index }) => {
    ...
    <Droppable key={"droppeable-panel-"+String(panel.id)} droppableId={String(panel.id)} index={index} direction="vertical">
      {(provided) => (
        <Paper ref={provided.innerRef} {...provided.droppableProps}  style={panelStyle}>
            <TitlePanel panel={panel} isEditable={true}/>
